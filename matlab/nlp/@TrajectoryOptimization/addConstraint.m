function obj = addConstraint(obj, label, nodes, cstr_array)
    ...
    if isa(cstr_array,'NlpFunctionSum')
        constr = repmat(NlpFunctionSum(),obj.NumNode,1);
        for j=1:numel(node_list)
            constr(node_list(j)) = cstr_array(j);
        end
    elseif isa(cstr_array,'NlpFunction')
    ...
    elseif isstruct(cstr_array)
        ...
        if isfield(cstr_array,'DependentFuncs')
            constr = repmat(NlpFunctionSum(),obj.NumNode,1);
            for j=1:numel(node_list)
                constr(node_list(j)) = NlpFunctionSum(cstr_array(j));
            end
        else
            constr = repmat(NlpFunction(),obj.NumNode,1);
            for j=1:numel(node_list)
