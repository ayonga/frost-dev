function obj = updateVariableProp(obj, label, node, varargin)
    ...
    if ischar(node)
        switch node
            case 'first'
                updateProp(obj.OptVarTable.(label)(1), varargin{:});
            case 'last'
                updateProp(obj.OptVarTable.(label)(end), varargin{:});
            case 'all'
                for i=1:obj.NumNode
                    updateProp(obj.OptVarTable.(label)(i), varargin{:});
                end
            case 'cardinal'
