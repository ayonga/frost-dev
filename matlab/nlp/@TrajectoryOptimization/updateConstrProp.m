function obj = updateConstrProp(obj, label, node, varargin)
    ...
    if ischar(node)
        switch node
            case 'first'
                updateProp(obj.ConstrTable.(label)(1), varargin{:});
            case 'last'
                updateProp(obj.ConstrTable.(label)(end), varargin{:});
            case 'all'
                for i=1:obj.NumNode
                    updateProp(obj.ConstrTable.(label)(i), varargin{:});
                end
            case 'cardinal'
    ...
    % obj.ConstrTable{name, :} = constr_array;
end