...
for i = 1:model.NB
  ...
  Ic(:,:,i) = model.I(:,:,i);
  hc(:,:,i) = Ic(:,:,i) * v(:,:,i);
  KE(i) = 0.5 * transpose(v(:,:,i)) * hc(:,:,i);
end

ret.Itot = zeros(nBase, nBase);
ret.htot = zeros(nBase, 1);
...
for i = model.NB:-1:1
  if model.parent(i) ~= 0
    ...
    hc(:,:,model.parent(i)) = hc(:,:,model.parent(i)) + transpose(Xup(:,:,i))*hc(:,:,i);
  else
    ret.Itot = ret.Itot + transpose(Xup(:,:,i))*Ic(:,:,i)*Xup(:,:,i);
    ret.htot = ret.htot + transpose(Xup(:,:,i))*hc(:,:,i);
  end
end
