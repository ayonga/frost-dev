function [Behavior] = pickup_and_flip_burger_behavior(flippy,cur,varargin)
...
elseif numargin < 2
    ...
    burger_index = 1; % corresponding to the index of the input arguments of the function
else
    ...
    burger_index = 2; % corresponding to the index of the input arguments of the function
end

if isfield(varargin{burger_index},'position')
    p_end = varargin{burger_index}.position;
    if p_end < boxes.grill_box.p_min(3)
...
if isfield(varargin{burger_index},'orientation')
    o_end = varargin{burger_index}.orientation;
else
    o_end = [0,0,0];
end
...
Behavior = validatePickAndFlipBehaviorBox(Behavior, boxes, pose_home, pose_start, pose_end);
