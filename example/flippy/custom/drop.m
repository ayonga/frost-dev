function nlp = drop(nlp, bounds, varargin)
if numel(varargin)<1
    p_drop = [-0.2, 0.8, 0.25]; % this is point A
    %%  Specify the starting and the ending orienation
    o_drop  = [0,0,pi/2];
else
    pose_drop = varargin{1};
    p_drop = pose_drop.position;
    o_drop = pose_drop.orientation;
    ...
    p_start = p_drop;
    o_start = o_drop;
    p_int = p_start - [0.02 0 0];
    p_end   = p_start + [0.04 0 0];
    o_int = o_start;
    o_end = o_start;
...
%%
    ...
    %% configuration constraints
    if ~isempty(qstart)
        q_init_L = qstart -0.02;
        q_init_U = qstart +0.02;
...
%% This is x position of end effector
    ...
    addNodeConstraint(nlp, p_x_func, {'x'}, n_int, p_int(1), p_int(1), 'Nonlinear');
...
%% This is y position of end effector
    ...
    addNodeConstraint(nlp, p_y_func, {'x'}, n_int, p_int(2), p_int(2), 'Nonlinear');
...
%% This is z position of end effector    
    ...
    addNodeConstraint(nlp, p_z_func, {'x'}, n_int, p_int(3), p_int(3), 'Nonlinear');
...
%     velocity functions
    ...
    addNodeConstraint(nlp, vx_func, {'x','dx'}, 1:n_int, -0.3, 0.3, 'Nonlinear');
    addNodeConstraint(nlp, vy_func, {'x','dx'}, 1:n_int, -0.3, 0.3, 'Nonlinear');
    addNodeConstraint(nlp, vz_func, {'x','dx'}, 1:n_int, -0.3, 0.3, 'Nonlinear');
    ...
    addNodeConstraint(nlp, slipping_func, {'x','dx','ddx'}, 1:n_int, cos(mu), 1, 'Nonlinear');

    addNodeConstraint(nlp, slipping_func, {'x','dx','ddx'}, n_int+1:n_node, 0, 0.01, 'Nonlinear');
