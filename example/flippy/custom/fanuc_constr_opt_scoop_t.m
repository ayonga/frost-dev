function nlp = fanuc_constr_opt_scoop_t(nlp, bounds, varargin)
    
%%  Specifications for the scoop
    grill_height = 0.085;
    spatula_length = 0.127;
    approach_offset = 0.127;
    approach_tilt = 0.314;
...
%% spatula front edge should be at a constant height 
    spatula_edge_z = p_z - spatula_length*sin(o_y)/2;
    spatula_edge_z_func = SymFunction(['spatulaedgez_' plant.Name],spatula_edge_z,{x});
    addNodeConstraint(nlp, spatula_edge_z_func, {'x'}, 'all', p_burger_location(3)-0.01, p_burger_location(3)+0.01, 'Nonlinear');
