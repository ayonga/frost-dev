...
nlp.setOption('StackVariable',true);

x_s = BoundedVariable('x',[3,1], -100, 100); %name, dim, lb, ub
y_s = BoundedVariable('y',[2,1], -100, 100);
...
costs = repmat(NlpFunction(),1,1); %preallocation
...
c1 = x_s(1) +  sin(3*x_s(2));
c2 = x_s(3) + cos(y_s(1)) - 2*y_s(2);
c3 = tovector(x_s(2) * y_s(2));
...
constrs(2) = NlpFunction(f_constr2,[x_var;y_var],'lb',0,'ub',5);

constrs(3) = NlpFunction(f_constr3,[x_var;y_var],'lb',-3,'ub',4);
