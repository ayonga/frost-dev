% start_vy = 0.0;
start_vy = [-0.6, -0.4, -0.2, 0, 0.2, 0.4, 0.6];
target_vy = [0.0];
% start_vx = 0.0;
...
vx_all2 = cell(N_vx, N_vy);
vy_all2 = cell(N_vx, N_vy);
alpha_all = cell(N_vx, N_vy);
alpha_all2 = cell(N_vx, N_vy);
for i = 1:N_vx
    vx = start_vx(i);
    for j = 1:N_vy
        vy = start_vy(j);
        ...
        if ~plot_single_step
            ...
            t_all{i,j} = [param.gait(1).tspan];%, param.gait(3).tspan];
			t_all2{i,j} = [param.gait(3).tspan];%, param.gait(3).tspan];
            q_all{i,j} = [param.gait(1).states.x];%, param. gait(3).states.x];
            dq_all{i,j} = [param.gait(1).states.dx];%, param. gait(3).states.dx];
			...
			vx_all2{i,j} = [param.gait(3).states.dx(1,:)];%ones(size(t_all{i,j}))*vx;
            vy_all2{i,j} = [param.gait(3).states.dx(2,:)];%ones(size(t_all{i,j}))*vy;
			alpha_all{i, j} = [param.gait(1).params.aoutput];
			alpha_all2{i, j} = [param.gait(3).params.aoutput];
        end
    end
...
%% Save Training Data
save('Ayonga3DgridTransitionMiddleGaits', 'alpha_all','alpha_all2', 'start_vy','start_vx','t_all','vx_all','vy_all','t_all2','vx_all2','vy_all2')
% save('Ayonga3DgridTransitionMiddleGaits', 'q_all','start_vy','start_vx','t_all','vx_all','vy_all')
