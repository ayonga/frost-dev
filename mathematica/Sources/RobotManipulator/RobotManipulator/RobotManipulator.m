...
ComputeSpatialPositions::usage = 
	"ComputeSpatialPositions[{link1,offset1},...,{link$n,offset$n},robotKinematics] computes \
...
GravityVector[q_,robotLinks_,robotKinematics_] :=
	Block[{V, ge},
		...
		ge = Vec[D[Flatten[V],{Flatten[q],1}]];
...
ComputeBodyJacobians[args__,robotKinematics_] :=
	Block[{gs0,i,np,Jz,Je,curIndices,linkName,offset,jIndex,pIndices,gst0,kinTwists,chainIndices,
		...
		Table[		
			...
			gs0 = gst0[[jIndex]].RPToHomogeneous[I3, offset];
			...
			If[jIndex != 0, (* the link is not the base link *)
				...
				Jz[[;;,curIndices]]=BodyJacobian[Sequence@@kinTwists[[jIndex]], gs0];
			];
...
ToEulerAngles[gst_,q_] :=
	Block[{R, R0, Rw, yaw, roll, pitch,q0subs},
		...
		q0subs = Join[Sequence @@ Map[Association[#[[1]] -> 0] &, Flatten@q]];
		R0 = R/.q0subs;
...
ComputeRigidPositions[args__,robotKinematics_] :=
	Block[{pos, gst},
...
ComputeSpatialPositions[args__,robotKinematics_,q_] :=
	Block[{pos, gst},
		...
		(* compute rigid positions *)
		pos = Map[Join[RigidPosition[#],ToEulerAngles[#,q]]&,gst];
	...
	Block[{pos, argList = {args}, np, i, Je, pIndices,gst0,kinTwists,chainIndices,
		...
		Je = Table[		
			...
			If[jIndex != 0, (* the link is not the base link *)				
				...
				Jz[[;;,curIndices]]=SpatialJacobian[Sequence@@kinTwists[[jIndex]], gs0];
			];
	...
	Block[{i,np,gst,gs0,linkName,offset,jIndex,pIndices,gst0,kinTwists,
		...
		Table[		
			...
			If[jIndex == 0, (* the link is the base link *)
				gst[i]=gs0;
				,
				gst[i]=ForwardKinematics[Sequence@@kinTwists[[jIndex]], gs0];
			];
...
GetKinematicTree[robotJoints_] := 
	Block[{childIndex,link,jointIndex,terminals,numBranch,tree,i,j, tIndex}, 
		...
		childIndex = GetFieldIndices[robotJoints, "child"];
